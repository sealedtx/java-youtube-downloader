...
public abstract class AbstractVideo<T extends AbstractVideoDetails> {

    private T videoDetails;
    protected List<Format> formats;
    protected List<SubtitlesInfo> subtitlesInfo;

    public AbstractVideo(T videoDetails) {
    ...
    public Format findFormat(Predicate<Format> filter) {
        for (Format format : formats()) {
            if (filter.test(format)) {
                return format;
            }
        }
        return null;
    ...
    public Future<File> downloadAsync(Format format, File outDir) throws DownloadUnavailableException, IOException {
        return downloadAsync(format, outDir, videoDetails.title());
    }

    public Future<File> downloadAsync(Format format, File outDir, String fileName) throws DownloadUnavailableException, IOException {
        return downloadAsync(format, outDir, fileName, false);
    }

    public Future<File> downloadAsync(final Format format, final File outDir, final String fileName, final boolean overwrite) throws DownloadUnavailableException, IOException {
        videoDetails.checkDownload();
    ...
    public void downloadAsync(Format format, File outDir, OnYoutubeDownloadListener listener) throws IOException, DownloadUnavailableException {
        downloadAsync(format, outDir, videoDetails.title(), listener);
    }

    public void downloadAsync(Format format, File outDir, String fileName, OnYoutubeDownloadListener listener) throws IOException, DownloadUnavailableException {
        downloadAsync(format, outDir, fileName, false, listener);
    }

    public void downloadAsync(Format format, File outDir, String fileName, boolean overwrite, final OnYoutubeDownloadListener listener) throws IOException, DownloadUnavailableException {
        videoDetails.checkDownload();
